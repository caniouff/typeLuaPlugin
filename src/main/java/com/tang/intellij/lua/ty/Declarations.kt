...
private fun LuaTableField.infer(context: SearchContext): ITy {
    ...
    if (valueExpr is LuaLiteralExpr && valueExpr.kind == LuaLiteralKind.String ) {
        ...
        if (typeFromValue != null) {//check is struct field or not
            val tbl = PsiTreeUtil.getParentOfType(this, LuaTableExpr::class.java)
            val call = PsiTreeUtil.getParentOfType(tbl, LuaCallExpr::class.java)
            val callType = inferExpr(call, context)
            if (TyUnion.find(callType, TyStruct::class.java) != null) {
