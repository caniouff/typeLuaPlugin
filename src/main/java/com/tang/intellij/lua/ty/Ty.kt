...
class TyFuncDef : Ty(TyKind.FuncDef) {
    ...
    fun toNamedFunc(name:String): ITy {
        val moduleName = moduleName
        if (global) {
            return union(TyClass.createGlobalType(name))
        } else if (moduleName != null) {
            return union(TyLazyClass(moduleName))
        }
        return this
    }

    fun toFuncDefReceiver() : ITy {
        val moduleName = moduleName
        if (global) {
            return union(TyClass.createGlobalType(Constants.WORD_G))
        } else if (moduleName != null) {
            return TyLazyClass(moduleName)
        }
        return receiver ?: Ty.UNKNOWN
    }
}
